...
export default function ConfirmBorrowPage() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data.success && data.book) {
            ...
            setBookDetails({
              id: book.id,
              title: book.title || decodeURIComponent(bookTitle),
              subtitle: book.description ? book.description.substring(0, 80) + '...' : '',
              author: book.author || decodeURIComponent(author),
              rating: book.rating || 0,
              reviews: book.totalReviews || 0,
              cover: book.cover || book.coverUrl || `https://via.placeholder.com/300x400/667eea/ffffff?text=${encodeURIComponent(bookTitle.substring(0, 10))}`,
        ...
        setBookDetails(prev => prev || {
          ...
          rating: 0,
          reviews: 0,
          cover: `https://via.placeholder.com/300x400/667eea/ffffff?text=${encodeURIComponent(bookTitle.substring(0, 10))}`,
          categories: ['General']
        });
        setBookLoading(false);
      }
  ...
  const safeBookTitle = (bookDetails?.title || decodeURIComponent(bookTitle) || 'Unknown Book');
  const safeBookAuthor = (bookDetails?.author || decodeURIComponent(author) || 'Unknown Author');
